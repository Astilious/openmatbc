...
class Task(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.parameters = {
            ...
            'nextfrequencyoverride': 0.0,
            'nextradiooverride': '',
            'network': "no"
        }
        
        # Add variables for focably updating the radio frequencies.
        self.parameters['updatefrequency'] = {}
        for radio in self.parameters['promptlist']:
            self.parameters['updatefrequency'][radio] = 0.0
    ...
    def onStart(self):
        ...
        self.callsign.setText(
            u'Callsign    \u27A1    ' + self.parameters['owncallsign'])
    ...
    def onUpdate(self):
        ...
        # Check for forced radio frequency changes.
        for radio, frequency in self.parameters['updatefrequency'].items():
            if frequency != 0.0:
                radio_id = self.parameters['promptlist'].index(radio)
                self.parameters['radios']['own'][radio_id]['currentfreq'] = \
                                                                       frequency
                self.parameters['radios']['own'][radio_id]['ui'].refreshValues()
                self.parameters['updatefrequency'][radio] = 0.0
        ...
        if self.parameters['radioprompt'].lower() in ['own', 'other']:
            ...
            while True:
                if self.parameters['nextradiooverride'] == '':
                    selected_radio = choice(self.parameters['promptlist'])
                    if selected_radio != self.lastradioselected:
                        break
                else:
                    selected_radio = self.parameters['nextradiooverride']
                    self.parameters['nextradiooverride'] = ''
                    break

            self.lastradioselected = copy.copy(selected_radio)
            ...
            for this_destination in self.parameters['radios']:
                for this_radio, radio_values in \
                    ...
                    if (radio_values['targetfreq'] !=
                        ...
                        if this_destination == 'own':
                            callsign = self.parameters['owncallsign'] 
                        else:
                            if self.parameters['nextcallsignoverride'] == '':
                                callsign = \
                                        choice(self.parameters['othercallsign'])
                            else:
                                callsign = \
                                         self.parameters['nextcallsignoverride']
                                self.parameters['nextcallsignoverride'] = ''
                        ...
                        radio_values['lasttarget'] = \
                            copy.copy(radio_values['targetfreq'])
    ...
    def getSyncData(self):
        ...
        # Get the radio channel settings.
        radio_data = {"own": {}}
        radios = self.parameters["radios"]["own"]
        for radio_name, radio_values in radios.items():
            radio_data["own"][radio_name] = {}
            radio_data["own"][radio_name]["currentfreq"] = radio_values["currentfreq"]
    ...
    def applySyncData(self, syncData, full_update=True):
        ...
        if full_update:
            ...
            # Syncronise the radio settings.
            for radio_name, radio_data in syncData["radio_data"]["own"].items():
                lastfreq = self.parameters["radios"]["own"][radio_name]["currentfreq"]
                self.parameters["radios"]["own"][radio_name]["currentfreq"] = radio_data["currentfreq"]
                if lastfreq != radio_data["currentfreq"]:
    ...
    def popNewInputs(self):
        ...
        self.input_record["radio_changes"] = []
        return temp


    def applyNewInputs(self, new_inputs):

        # Calculate the total radio changes.
        radio_changes = new_inputs["radio_changes"]
        total_changes = {index : 0.0 for index in
                                        self.parameters["radios"]["own"].keys()}
        for event in radio_changes:
            total_changes[event[0]] += event[1]

        # Apply the new radio changes.
        for index, change in total_changes.items():
            if change != 0.0:
                radio = self.parameters["radios"]["own"][index]
                radio["currentfreq"] += total_changes[index]
                radio['currentfreq'] = self.roundFrequency(radio['currentfreq'])
                radio['ui'].refreshValues()
                self.buildLog(["STATE", 'OWN', radio['name'], "CLIENT",
                                                     str(radio['currentfreq'])])
    ...
    def keyEvent(self, key_pressed):
        ...
        if key_pressed not in [QtCore.Qt.Key_Up, QtCore.Qt.Key_Down,
            ...
            if next_radio is not None:
                ...
                self.buildLog(["STATE", 'OWN', next_radio, "SELECTED"], local_log=True)
            ...
            if (key_pressed == QtCore.Qt.Key_Right and
                ...
                lastfreq = copy.copy(selected_radio['currentfreq'])
            ...
            if lastfreq != selected_radio['currentfreq']:
                selected_radio['ui'].refreshValues()
                if self.is_client() and lastfreq is not None:
                    self.input_record["radio_changes"].append((selected_radio["index"], selected_radio['currentfreq'] - lastfreq))
                elif self.is_server() and lastfreq is not None:
                    self.buildLog(["STATE", 'OWN', selected_radio['name'], "SERVER",
                              str(selected_radio['currentfreq'])])
                elif lastfreq is not None:
    ...
    def setTargetFrequency(self, prompt_destination, radio_name):
        radio = [r for i, r in
                 ...
                 r['name'] == radio_name][0]
        if self.parameters['nextfrequencyoverride'] == 0.0:
            good_frequency = False
            while not good_frequency:
                selected_frequency = self.generateFrequency()
                if (self.parameters['airbandminvariationMhz'] <
                        abs(selected_frequency - radio['currentfreq']) <
                        self.parameters['airbandmaxvariationMhz']):
                    good_frequency = True
        else:
            selected_frequency = self.parameters['nextfrequencyoverride']
            self.parameters['nextfrequencyoverride'] = 0.0
            
        radio['targetfreq'] = selected_frequency
    ...
    def create_audio_file(self, output_name):
        ...
        # I don't understand why, but the audio plays at double speed so the 
        # framerate must be halved.
        output.setframerate(data[0][2] / 2) 
    ...
    def buildLog(self, thisList, local_log=False):
        thisList = ["COMMUN"] + thisList
        if not local_log and self.is_server():
            self.parent().mainLog.addLine(thisList)
            self.event_record.append(thisList)
        elif local_log or self.is_local():
            self.parent().mainLog.addLine(thisList)
